 ...
 * based on libdragon, Neo64Menu, ED64PIO, libn64-hkz, libmikmod
 * by saturnu
 */
...
void configure() {
    ...
    if (streql("ED64P SD boot", buff, 12) && firm >= 0x0116) {
        sd_mode = 1;
    }
...
void romInfoScreen(display_context_t disp, u8 *buff, int silent){
    ...
    if(silent!=1){
        ...
        sprintf(box_path, "/ED64P/boxart/lowres/%c%c.png",  headerdata[0x3C], headerdata[0x3D]);
        ...
        if(found!=0){
            //not found
            sprintf(box_path, "/ED64P/boxart/lowres/00.png");
        }
...
void loadgbrom(display_context_t disp, u8 *rom_path){
    FatRecord rec_tmpf;
    ...
    if (exist == 0) {
        ...
        u8 resp=0;
        //load gb emulator
        resp = fatOpenFileByeName("/ED64P/gb.v64", 0); //err if not found ^^
...
void loadmsx2rom(display_context_t disp, u8 *rom_path){
    ...
    u8 exist=0;
    exist = fatFindRecord("/ED64P/ultraMSX2.z64", &rec_tmpf, 0); //filename already exists

    if (exist == 0) {
        u8 resp=0;
        //load nes emulator
        resp = fatOpenFileByeName("/ED64P/ultraMSX2.z64", 0); //err if not found ^^
...
void loadggrom(display_context_t disp, u8 *rom_path){
    ...
    u8 exist=0;
    exist = fatFindRecord("/ED64P/UltraSMS.z64", &rec_tmpf, 0); //filename already exists

    if (exist == 0) {
        u8 resp=0;
        //load nes emulator
        resp = fatOpenFileByeName("/ED64P/UltraSMS.z64", 0); //err if not found ^^
...
void loadnesrom(display_context_t disp, u8 *rom_path){
    ...
    if (exist == 0) {
        ...
        u8 resp=0;
        //load nes emulator
        resp = fatOpenFileByeName("/ED64P/neon64bu.rom", 0); //err if not found ^^
        resp = diskRead(file.sector, (void *)0xb0000000, file.sec_available);
...
int backupSaveData(display_context_t disp){
        ...
        sprintf(fname, "/ED64P/%s/LASTROM.CFG",save_path);
...
int saveTypeFromSd(display_context_t disp, char* rom_name, int stype) {
    ...
    sprintf(fname, "/ED64P/%s/%s.%s",save_path,rom_name, saveTypeToExtension(stype,ext_type));
...
int saveTypeToSd(display_context_t disp, char* rom_name ,int stype) {
    ...
    u8 tmp[32];
    u8 fname[256]; //filename buffer to small :D
    u8 found=0;
...
int readConfigFile(void){
    ...
    //config filename
    sprintf(filename, "/ED64P/ALT64.INI");
    FatRecord rec_tmpf;
    ok = fatFindRecord(filename, &rec_tmpf, 0);
...
void readSDcard(display_context_t disp, char *directory){
    ...
    cresp = fatLoadDirByName("/ED64P/CFG");
    ...
    if(enable_colored_list){
        for (int i = 0; i < dir->size; i++){
            ...
            //set rom_cfg
            sprintf(rom_cfg_file, "/ED64P/CFG/%s",frec->name );
    ...
    for (int i = 0; i < dir->size; i++) {
        ...
        if(strcmp(rec->name,"ED64P")==0 && hide_sysfolder==1){
            //don't add
        }
        else{
            if(rec->is_dir){
...
void bootRom(display_context_t disp, int silent) {
    if (boot_cic != 0) {
        if (boot_save != 0){
            ...
            sprintf(cfg_file, "/ED64P/%s/LASTROM.CFG", save_path);
...
void readRomConfig(display_context_t disp, char* short_filename, char* full_filename){
    ...
    sprintf(cfg_filename,"/ED64P/CFG/%s.CFG",rom_filename);
...
void drawToplistBox(display_context_t disp,int line){
    ...
    if(line==0){
        ...
        //load the directory-entry
        resp = fatLoadDirByName("/ED64P/CFG");
        ...
        for (int i = 0; i < dir->size; i++){
            ...
            //set rom_cfg
            sprintf(rom_cfg_file, "/ED64P/CFG/%s",rec->name );
...
void drawRomConfigBox(display_context_t disp,int line){
    ...
    switch(rom_config[1]){
        ...
        case 3:    printText("     CIC: 5101", 9, -1, disp); break; //do i really need 6104 in that list? :D
        case 4:    printText("     CIC: 6105", 9, -1, disp); break;
        case 5:    printText("     CIC: 6106", 9, -1, disp); break;
        case 6:    printText("     CIC: 5167", 9, -1, disp); break;
        default: break;
    }
...
void drawSet1(display_context_t disp){
    ...
    uint32_t     forecolor;
    uint32_t     backcolor;
    backcolor = graphics_make_color(0x00, 0x00, 0x00, 0x00);
        ...
        while(1) {
            ...
            if(keys.c[0].down || keys_held.c[0].down || keys_held.c[0].y < -25) {
                ...
                if(input_mapping==file_manager){
                    ...
                    if(quick_boot){
                        ...
                        if(fatFindRecord("/ED64P/LASTROM.CFG", &rec_last, 0)==0){
                            u8 resp = 0;
                            resp = fatOpenFileByeName("/ED64P/LASTROM.CFG", 0);
                        ...
                        if(!strcmp(extension,"MPK" ))
                            ft=2;
                        if(!strcmp(extension,"GB" ) || !strcmp(extension,"GBC" ) || !strcmp(extension,"SGB" ))
                            ft=5;
                        if(!strcmp(extension,"NES" ))
                            ft=7;
                        if(!strcmp(extension,"GG" ))
                            ft=8;
                        if(!strcmp(extension,"MSX" ) || !strcmp(extension,"ROM" ))
                            ft=9;
                ...
                else if(input_mapping==mpk_format){ // format mpk
                    while (!(disp = display_lock()))
                        ;
                    new_scroll_pos(&cursor, &page, MAX_LIST, count);
                    clearScreen(disp); //part clear?
                ...
                else if(input_mapping==mpk_restore){ //restore mpk
                    while (!(disp = display_lock()))
                        ;
                    new_scroll_pos(&cursor, &page, MAX_LIST, count);
                    clearScreen(disp); //part clear?
                ...
                else if(input_mapping==mpk_quick_backup){ //quick-backup
                    while (!(disp = display_lock()))
                        ;
                    new_scroll_pos(&cursor, &page, MAX_LIST, count);
                    clearScreen(disp); //part clear?
                ...
                if(input_mapping==file_manager){
                    if(input_mapping==file_manager && list[cursor].type != DT_DIR ){//open
                        char name_file[64];
                ...
                else if(input_mapping==control_screen){
                    input_mapping=none;
            ...
            if (list[cursor].type == DT_DIR && empty == 0)
            {
                ...
                display_show(disp);
                    ...
                    else if(input_mapping==file_manager && list[cursor].type != DT_DIR  && empty==0){//open
                        char name_file[256];
                        ...
                        if(ft==1){ //rom
                            ...
                            if(quick_boot){
                                u8 resp=0;
                                resp = fatCreateRecIfNotExist("/ED64P/LASTROM.CFG", 0);
                                resp = fatOpenFileByeName("/ED64P/LASTROM.CFG", 1); //512 bytes fix one cluster
                    ...
                    //save rom_cfg[] to
                    // /ED64P/CFG/Romname.cfg if not exist create
                    // if exist open/write
                    ...
                    //set rom_cfg
                    sprintf(rom_cfg_file, "/ED64P/CFG/%s.CFG",rom_filename );
                ...
                else if(input_mapping==delete_prompt){
